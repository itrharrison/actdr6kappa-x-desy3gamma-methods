...
for kappa_map_name in kappa_60_map_names:
    ...
    cl_decoupled_arr = np.empty([len(cl_decoupled), len(ell_bins.get_effective_ells())])
    for ibin in np.arange(1, nbins_tomo + 1):
        cl_decoupled_arr[ibin-1] = cl_decoupled[ibin]

    np.savetxt(f'./output/{kappa_map_name}_ellcl.dat', np.column_stack([ell_bins.get_effective_ells(), cl_decoupled_arr.T]))
