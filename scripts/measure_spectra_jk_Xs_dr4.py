...
jk_regions = pickle.load(open('jk_regions_dr4.pkl', 'rb'))['jk_regions']
...
# kappa_map_dr4 = enmap.read_map(os.path.join(data_dir, 'data/act_planck_dr4.01_s14s15_D56_lensing_kappa_baseline.fits'))
...
# kappa_mask_dr4 = np.where(kappa_mask_dr4 > 1, 1, kappa_mask_dr4)
# kappa_mask_dr4 = np.where(kappa_mask_dr4 < 1e-2, 0, kappa_mask_dr4)
...
for ireg, region_tag in enumerate(region_tag_list):
    ...
    for ibin in np.arange(1,5):
        ...
        comb_mask = copy.copy(kappa_mask * gamma_mask)

        gamma_mask = np.where(wt_map == hp.UNSEEN, 0, wt_map)

        stellar_field = nmt.NmtField(comb_mask * region_mask, [stellar_map], beam=None, masked_on_input=False, lmax=elmax)#, purify_b=False, beam=None)
        ext_field = nmt.NmtField(comb_mask * region_mask, [ext_map], beam=None, masked_on_input=False, lmax=elmax)#, purify_b=False, beam=None)
        ...
        gamma_field = nmt.NmtField(gamma_mask * region_mask, [-1.0 * g1_map, g2_map], purify_b=False, beam=None, lmax=elmax)#, purify_b=False, beam=None)
        print('Done.')
        ...
        outfile = f'./data/region{region_tag}_Xs_spectra_bin{ibin}_dr4.pkl'
